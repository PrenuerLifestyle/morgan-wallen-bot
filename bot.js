// Morgan Wallen Personal Assistant Bot - FULL FEATURED VERSION
// Install: npm install telegraf express pg stripe dotenv nodemailer googleapis axios node-cron bull ioredis jsonwebtoken bcrypt
...
📅 ${new Date(t.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
🕐 Doors: ${t.doors_open || 'TBA'} | Show: ${t.show_start || 'TBA'}

📍 *${t.venue}*
${t.venue_address || ''}
${t.city}, ${t.state} ${t.country}

🎫 *Tickets Available:* ${t.tickets_available - t.tickets_sold}

💰 *Pricing:*
General: $${(t.ticket_price * (1 - discount)).toFixed(2)}${discount > 0 ? ` (${(discount*100)}% member discount!)` : ''}
VIP: $${(t.vip_price * (1 - discount)).toFixed(2)}${discount > 0 ? ` (${(discount*100)}% member discount!)` : ''}
